...
options = {'Twin Bar': '50p', 'Mar Bar': '20p', 'Glar Bar': '10p', 'Foo Bar': '5p', 'Coke': '3p'}
message = ['insert a coin', 'choose a candy or drink', 'coin accepted', 'current balance: ', 'Transaction cancelled']
# ============================================
#: 2.2 Functions
...
def update_options():
    candy_list = []
    if balance >= 50:
        candy_list.extend(['Twin Bar', 'Mar Bar', 'Glar Bar', 'Foo Bar', 'Coke'])
        dropdown_menu.set('Twin Bar')
    elif balance >= 20:
        candy_list.extend(['Mar Bar', 'Glar Bar', 'Foo Bar', 'Coke'])
        dropdown_menu.set('Mar Bar')
    elif balance >= 10:
        candy_list.extend(['Glar Bar', 'Foo Bar', 'Coke'])
        dropdown_menu.set('Far Bar')
    elif balance >= 5:
        candy_list.extend(['Foo Bar', 'Coke'])
        dropdown_menu.set('Foo Bar')
    elif balance >= 3:
        candy_list.extend(['Coke'])
        dropdown_menu.set('Coke')
    else:
        candy_list.extend([''])
    dropdown_menu['values'] = candy_list
    if balance <= 0:
        change_again()
        candy_list.extend(['50p', '20p', '10p', '5p', '3p'])
        dropdown_menu.set('50p')  # Set default option to 50p
        dropdown_menu['values'] = candy_list  # Update the dropdown options
        ...
        pass
def coinNoise():
    playsound('/home/leonardschwartz/Desktop/Personal/Codeing/Python/Vending-Machine-Simulator-main/coin.mp3')

#: 2.2.4 Update instructions
def update_instructions():
    show_instructions.configure(font=t_h1, text=message[1].upper())
...
def change_again():
    button_confirm.configure(command=lambda: confirm_1())
...
def status_candy():
    playsound('/home/leonardschwartz/Desktop/Personal/Codeing/Python/Vending-Machine-Simulator-main/Vend.mp3')
    messagebox.showinfo('Dispenced', f'Current balance:{balance}p\nDispensed: {dropdown_menu.get()}')
    playsound('/home/leonardschwartz/Desktop/Personal/Codeing/Python/Vending-Machine-Simulator-main/CHOMP.mp3')

def close():
    messagebox.showinfo('Transaction Complete', 'Thank you.')
    playsound('/home/leonardschwartz/Desktop/Personal/Codeing/Python/Vending-Machine-Simulator-main/ching.mp3')
